...
(define-type defrdSub
  [mtSub]
  [aSub (namd id?) (value integer?) (rest defrdSub?)])
...
(define (store-lookup addr st)
  (type-case Store st
    ...
    [aSto (sto-a sto-v sto-rest) (if (= addr sto-a) sto-v (store-lookup addr sto-rest))]))
...
(define (reset-box st addr val)
  (type-case Store st
    ...
    [aSto (sto-a sto-v sto-rest) (if (= addr sto-a)
                                     (aSto addr val sto-rest)
  ...
  (cond
    ...
    [else (local [(define a (malloc st))]
            (recording-id (rest namelist) (rest valuelist)
                          (aSub (first namelist) a r2ds) (aSto a (v*s-value (first valuelist)) st)))]))
...
(define (interp bwae ds st)
  (type-case BWAE bwae
    ...
    [openbox (b) (type-case Value*Store (interp b ds st)
                   [v*s (b-cont st1)
                        (v*s (store-lookup (boxV-address b-cont) st1) st1)])]
    [setbox (b val) (interp-two b val ds st
                                (lambda (v1 v2 st1) (v*s v2 (reset-box st1 (boxV-address v1) v2))))]
                                   ...
                                   (local [(define a (malloc st1))
                                           (define interpV
                                             (interp (closureV-body ft-val)
                                                     (aSub (closureV-param ft-val) a (closureV-ds ft-val))
                                                     (aSto a arg-val st1)))
